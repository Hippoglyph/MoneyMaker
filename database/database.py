...
from database.file_log_entry import FileLogEntry
...
class Database:
    ...
    def create_table_if_not_exists():
        """Creates the file_logs table if it doesn't already exist."""
            ...
            try:
                ...
                cursor.execute('''
                    CREATE TABLE IF NOT EXISTS file_logs (
                        ...
                        uploaded_youtube TEXT DEFAULT NULL
                    )
                ''')
                conn.commit()
                print(f"Database table 'file_logs' ensured in '{DATABASE_NAME}'.")
            except sqlite3.Error as e:
    ...
    def log_file_upload_info(description: str, filename: str):
        ...
        try:
            ...
            cursor.execute(
                '''
                INSERT INTO file_logs (description, filename, creation_timestamp, uploaded_youtube)
                VALUES (?, ?, ?, ?)
                ''',
                (description, filename, current_timestamp, None) # uploaded_youtube is initially NULL
            )
            conn.commit()
    ...
    def view_all_logs():
        """Retrieves and prints all entries from the file_logs table."""
        conn = Database.get_db_connection()
        if conn is None:
            return
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT id, description, filename, creation_timestamp, uploaded_youtube FROM file_logs ORDER BY creation_timestamp DESC")
            ...
            for row in rows:
                ...
                log_entry = FileLogEntry(**row) # **row unpacks the dict-like Row object into keyword arguments
    ...
    def get_pending_youtube_uploads() -> list[FileLogEntry]:
        ...
        returned as a list of FileLogEntry objects.
        ...
        try:
            cursor = conn.cursor()
            # Select all columns where 'uploaded_youtube' is NULL
            cursor.execute("SELECT id, description, filename, creation_timestamp, uploaded_youtube FROM file_logs WHERE uploaded_youtube IS NULL ORDER BY creation_timestamp ASC")
            rows = cursor.fetchall()

            for row in rows:
                # Instantiate FileLogEntry object directly from the sqlite3.Row object
                # The **row syntax unpacks the dictionary-like row into keyword arguments for __init__
